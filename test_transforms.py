...
def test_find_urls():
    ...
    assert transforms.find_urls(data)[0]=='http://abcde',"Found the url"
    assert transforms.find_urls(data[0:1])==[], 'No false urls'

def test_ymd_to_datetime():
    ymd = "2011-01-01"
    dt = transforms.ymd_to_datetime(ymd)
...
def test_text_to_ngrams():
    txt = "a b a b c"
    out = transforms.text_to_ngrams(txt,2)
    fr1 = {'a': 2,   'b': 2,   'c': 1}
